...
Navigate to the cloned Papy directory:
```bash
cd Papy
```

You can determine if you have Make installed by using:
```bash
make --version
```
...
### First Steps
Lets launch some requests! The following are examples of possible use cases and the commands that would go along with them. 
...
If you want to send a static payload from a JSON file then simply specify the file path to your JSON file. 
...
The following is an example of me testing the database of a locally deployed web application I created called [M-Track](https://github.com/noahpop77/M-Track). 

This execution run will use the randomized payload generation for League of Legends. 

The lol payload will be a randomized League of Legends match file and send that to the API endpoint of [M-Track](https://github.com/noahpop77/M-Track).
...
### --threads [num]
This parameter lets you specify the amount of CPU threads you want to use for the programs execution. The more you add the more concurrent requests will be sent. 
...
### --count [num]
This parameter will let you specify the amount of payloads that each thread will launch. 
...
- `lol`: If this is set then it will generate a custom pseudo-random payload. 
- `{filepath}`: If a file path is specified then Papy will look for a JSON file and parse it. That parsed JSON will then be used as a body for the payload.
...
When using WSL you might encounter issues which can be fixed by doing the following steps:
```bash git 
clone --depth 1 https://github.com/microsoft/WSL2-Linux-Kernel
```
```bash 
...
- Convert it to readeable format for flamegraph 
```bash 
./stackcollapse-perf.pl out.perf > out.folded
```
- Then create a visual out of the data 
```bash 
